...
%{
fprintf('%f %f %f %f',size(R),size(X),size(Theta),size(Y));
%}
...
for i=1:num_movies
	...
	Theta_temp=Theta(idx,:);  %{ containing only required elements %}
	Y_temp=Y(i,idx);
...
for i=1:num_movies
	sum=0;
	for j=1:num_users
		sum=sum+(Theta(j,:)*X(i,:)'-Y(i,j)).*R(i,j).*Theta(j,:);
	end;
		X_grad(i,:)=sum;
end;
...
for j=1:num_users
	sum=0;
	for i=1:num_movies
		sum=sum+ (Theta(j,:)*X(i,:)'-Y(i,j)).*R(i,j).*X(i,:);
	end;
	Theta_grad(j,:)= sum;
end;
